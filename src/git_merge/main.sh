...
if [[ $# -eq 6 ]]; then
  ...
  merge)
    ...
    # Preserve last commit of base branch
    ...
    rc=$?
    if [[ $rc -ne 0 ]]; then
      echo "err: \"git merge --no-ff $base_branch\" returned $rc"
      echo "rolling back temporary git config"
      rollback_gitconfig
      exit $rc
    fi
    ...
    # Commit
    git commit --amend --message="$commit_message" --author="$author"
    rc=$?
    if [[ $rc -ne 0 ]]; then
      echo "err: \"git commit --amend\" returned $rc"
      echo "unstaging and discaring all items in working tree..."
      rollback_commit $last_commit
      echo "rolling back temporary git config"
      rollback_gitconfig
      exit $rc
    fi
